...
export default ({ on, set }) => {
  ...
  on('= update', () => {
    ...
    return updateFromLibrary((pos, tot) => set('progressLibrary', p(pos, tot))).then(async books => {
      ...
      set('progressAuthors', authors.reduce((res, author) => {
        res[author] = 0;
        return res;
      }, {}));
      ...
      await Promise.all(
        range(0, 10).map(async index => {
          ...
          while (!!(author = authors.pop())) {
            await updateByAuthor(author, (pos, tot) => set('progressAuthor', {
              author,
              progress: p(pos, tot)
            }));
            authorsDone++;
            set('progressAllAuthors', Math.floor(authorsDone / authorsCount * 100));
          }
        })
      );
      save();

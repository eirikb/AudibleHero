...
import { Api, BookMap, Progress } from 'types';

const BOOKS_PR_PAGE = 50;

// const loadBooks = async (books: BookMap, progress: Progress, api: Api) => {
...
const load = async (type: string, progress: Progress, api) => {
...
export const getBooks = () => {
  ...
  const books = uniqBy(
    authorBooks.map(book => Object.assign({}, book, libraryById[book.id])),
    'id'
  );
  const series = books
    .filter(book => book.seriesId)
    .reduce((res, book) => {
  ...
  Object.values(series).forEach(books => {
    ...
    books.forEach(book => {
      ...
      book.seriesBookIndexInLibrary = books.some(
        b => b.inLibrary && b.seriesBookIndex === book.seriesBookIndex
      );
    });
  });
...
export const updateFromLibrary = (progress: Progress) =>
  load('library', progress, fromLibrary);

export const updateByAuthor = (author: string, progress: Progress) =>
  load(`author-${author}`, progress, (limit, pageIndex) =>

...
export default (
  page: number
): Promise<{ pageCount: number; books: LibraryBook[] }> =>
  fetch(
    `/lib?programFilter=all&purchaseDateFilter=all&pageSize=50&page=${page}&sortBy=PURCHASE_DATE.dsc`,
    {
      credentials: 'include',
    }
  )
    ...
    .then(html => {
      ...
      const books = rows.map(row => {
        const id = (row.querySelector('[name=asin]') as HTMLInputElement).value;

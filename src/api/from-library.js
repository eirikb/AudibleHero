...
export default (limit, page) => fetch(`/lib?progType=all&timeFilter=all&itemsPerPage=${limit}&page=${page}&sortType=down`, {
  credentials: 'include'
}).then(r =>
  r.text()
).then(html => {
  ...
  const books = rows.map(row => {
    ...
    const authors = Array.from(row.querySelectorAll('a[href^="/author/"]'))
